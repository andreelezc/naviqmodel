...
class Property(EvaluativeEntity):
    ...
    #
    #         # Filter out applications that have "N/A" selected by the user
    #         applications_to_consider = applications.exclude(id__in=[app.id for app in applications_with_na]).distinct()
    #
    #         # If there are no applications to consider, skip to the next ProfileCriterionProperty
    #         if not applications_to_consider.exists():
    #             continue
    #
    #         # Calculate weights: Only redistribute weights if applications are excluded because of "N/A"
    #         if applications.count() != applications_to_consider.count():
    #             weight_per_app = Decimal(1) / Decimal(applications_to_consider.count())
    #             weights = {app.id: weight_per_app for app in applications_to_consider}
    #         else:
    ...
    #         for app in applications_to_consider:
    #             try:
    ...
    #                 selected_options = user_response.selected_options.all()
    #                 app_score = app.calculate_score(selected_options)
    #                 if app_score is not None and app not in applications_with_na:
    #                     total_score += app_score * weights[app.id]
    #                     detailed_applications[app.name] = {
    #                         'score': str(app_score),
    #                         'weight': str(weights[app.id]),
